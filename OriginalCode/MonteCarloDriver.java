    ...
    private void processEvent(Event temp, PrintWriter out) throws IOException {
        ...
        if (particles[x][y][z] == null & !action.equals("excigen")) {
            ...
            if (particles[x][y][z] == null) {// If location is empty, generate exciton
                ...
                eventQueue.add(particles[x][y][z].nextEvent(getNeighbours(x, y, z), getCanvastype(x, y, z),
                        getEnergy(x, y, z)));
            ...
            printStuff(out);
            int[] loc = ExciGenerator.genExciton(xdim, ydim);
            Event exci = new Event(
                    (100000 / canvas.getGenrate(loc[0], loc[1], loc[2])) * -Math.log(RandomGenerator.randDouble()),
                    "excigen", loc[0], loc[1], loc[2], 0, 0, 0);
            eventQueue.add(exci);
            ...
            if (action.charAt(10) == 'A') {
                if (particles[x + dx][y + dy][z + dz] == null) {
                    ...
                    particles[x][y][z] = new Electron(x, y, z, canvas.getMaterialproperties(x, y, z));
                    particles[x + dx][y + dy][z + dz] = new Hole(x + dx, y + dy, z + dz,
                            canvas.getMaterialproperties(x + dx, y + dy, z + dz));
                    eventQueue.add(particles[x + dx][y + dy][z + dz].nextEvent(getNeighbours(x + dx, y + dy, z + dz),
                            getCanvastype(x + dx, y + dy, z + dz), getEnergy(x + dx, y + dy, z + dz)));
                    eventQueue.add(particles[x][y][z].nextEvent(getNeighbours(x, y, z), getCanvastype(x, y, z),
                            getEnergy(x, y, z)));
                    reset = false;
                }
                ...
                if (particles[x + dx][y + dy][z + dz] == null) {
                    ...
                    particles[x][y][z] = new Hole(x, y, z, canvas.getMaterialproperties(x, y, z));
            ...
            if (!checkInBounds(x + dx, y + dy, z + dz)) {
                ...
                if (type.equals("Hole")) {
                    ...
                    particles[x + dx][y + dy][z + dz] = new Exciton(x + dx, y + dy, z + dz, excipart.timealive,
                            canvas.getMaterialproperties(x + dx, y + dy, z + dz));
                    ;
